<table class="table table-bordered table-striped" style="min-width: 560px">
        <thead class="bg-default" >
            <tr>
                ...
                <th class="text-center" style="width: 20%; vertical-align: middle" >SUB</th>    
        ...
        <tbody>
            ...
            foreach ($show as $number => $row) :
                ?>
                <tr class="idc_<?php echo $row->msg_id; ?>">     
                    <td class="text-center"><?php echo $nom_started++ ; ?></td>
                    <td>
                        <i><?php echo indo_date($row->msg_date,1,1); ?></i>                        
                    </td>
                    <td>
                        <i><?php echo ucwords($row->msg_author); ?></i>                        
                    </td>
                    <td>
                        <i><?php echo ucwords($row->msg_mail); ?></i>                        
                    </td>
                    <td>
                        <i><?php echo ucwords($row->msg_subject); ?></i>                        
                    </td>
                    <td class="text-center">                        
                        <div class="btn-group btn-group-justified">   
                            <a role="button" class="label label-primary" onclick="action_control('detail', '<?php echo md5($row->msg_id); ?>')"  data-toggle="modal" data-target=".bs-example-modal-lg" title="Detail <?php echo strtolower($tit_param) ?>"><i class="fa fa-search"></i></a>&nbsp;
                            <a onclick="action_control('delete', '<?php echo md5($row->msg_id); ?>')" role="button" class="label label-danger" data-toggle="tooltip" data-placement="left" title="" title="Delete <?php echo strtolower($tit_param) ?>"><i class="fa fa-trash-o"></i></a>
                        </div>
                    </td>
                </tr>        
            <?php endforeach; ?>
