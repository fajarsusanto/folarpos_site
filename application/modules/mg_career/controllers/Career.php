...
class Career extends MY_Controller {
    ...
    private $url_index = "dash-manage/mg-career";
    private $title_param = "Career";
    ...
    public function index($el = null,$sort=null) {
        ...
        if (!empty($el)) {
            ...
            $jml = $this->db->query("select * FROM mg_contents where menu_id = 5 $sql_date $sql_");
            $per = 15;
            ...
            $data['show'] = $this->db->query("select * FROM mg_contents where menu_id = 5 $sql_date $sql_ LIMIT " . $offset . "," . $per)->result();

            $this->load->view("career_data", $data);
        } else {
            $data['content'] = "mg_career/career_index";
    ...
    public function form($el = null) {
        ...
        $this->load->view("career_form", $data);
    }
    ...
    public function save() {
        ...
        $this->form_validation->set_rules('date_publish', '', 'required');   
        $this->form_validation->set_rules('date_expired', '', 'required');     
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['cont_date'] = date('Y-m-d H:i:s', strtotime($this->input->post('date_publish')));
            $data['cont_date_expired'] = date('Y-m-d H:i:s', strtotime($this->input->post('date_expired')));
            $data['cont_status'] = 2;
            if (empty($_POST['id'])) {
