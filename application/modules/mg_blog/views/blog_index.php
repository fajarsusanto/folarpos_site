            ...
            <div class="col-sm-12">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h6 class="panel-title txt-dark"><i class="fa fa-th-large mr-10"></i>DATA Tips & Promo</h6>
                    </div>
...
<script>
    ...
    $('.applyBtn').on('click', function () {
        ...
        if (diffDays > 60) {
            ...
            $("#load-data").load('<?php echo base_url("dash-manage/mg-blog/data?range=") ?>' + tglnwx);
           // $("#load_anal").load('<?php echo base_url($url_index) ?>/load?range=' + tglnwx);
        }
        //return true;
    });
    ...
    $("#load-data").load('<?php echo base_url("dash-manage/mg-blog/data") ?>');
        function paging(e) {
            ...
            $("#load-data").load('<?php echo base_url("dash-manage/mg-blog/data") ?>/' + (selected ? selected : 'all') +'/'+ o);
        }

        function sorSup() {
            var selected = $("input[name=sort_data]").val().replace(" ", "-");
            $("#load-data").load('<?php echo base_url("dash-manage/mg-blog/data") ?>/' + (selected ? selected : 'all'));
