...
class Term extends MY_Controller {
    ...
    private $url_index = "dash-manage/mg-term";
    private $title_param = "Term";
    ...
    public function index() {
        ...
        $data['content'] = "mg_term/term_index";
    ...
    public function form() {
        ...
        $data['act'] = "EDIT";
        $data['tit_param'] = $this->title_param;
        $this->load->view("term_form", $data);
    }

    public function save() {
        $sess = $this->authentication_root();
        $this->form_validation->set_error_delimiters('<div class="alert alert-danger">', '</div>');
        $this->form_validation->set_rules('apps_term', '', 'required');
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['apps_term'] = $this->input->post('apps_term'); 
