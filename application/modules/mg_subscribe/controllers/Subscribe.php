...
class Subscribe extends MY_Controller {
    ...
    private $url_index = "dash-manage/mg-subscribe";
    private $title_param = "Subscribes";
    ...
    public function index($el = null,$sort=null) {
        ...
        if (!empty($el)) {
            $sql_ = (!empty($sort) ? ($sort != "all" ? " AND subs_name like '%" . (str_replace("-", " ", $sort)) . "%' " : null) : null);
            $jml = $this->db->query("select * FROM mg_subscribes where subs_id is not null $sql_");
            $per = 15;
            ...
            $data['show'] = $this->db->query("select * FROM mg_subscribes where subs_id is not null $sql_ LIMIT " . $offset . "," . $per)->result();

            $this->load->view("subscribe_data", $data);
        } else {
            $data['content'] = "mg_subscribe/subscribe_index";
    ...
    function detail($param = null) {
        if ($param != null) {
            $data['dt'] = $this->db->query("select * FROM mg_subscribes where md5(subs_id) = '$param'")->last_row();
            $this->load->view("subscribe_detail", $data);
        }
    }
    ...
    public function status($status = nul, $el = null) {
        if (!empty($el)) {
            ...
            if (!empty($checking)) {
                $this->session->set_flashdata('msg', '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><i class="fa fa-info-circle mr-10"></i> ' . $this->title_param . ' ' . ucwords($checking->users_fullname) . ' successfully ' . ($status == 2 ? "activated" : "disabled") . ' !</div>');
                $data['cont_id'] = $checking->users_id;
    ...
    public function delete($el = null) {
        if (!empty($el)) {
            $checking = $this->db->query("select * from mg_subscribes where md5(subs_id) = '" . $el . "'")->row();
            if (!empty($checking)) {
                $this->session->set_flashdata('msg', '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><i class="fa fa-info-circle mr-10"></i> ' . $this->title_param . ' ' . ucwords($checking->subs_name) . ' berhasil dihapus !</div>');
                $this->crud_model->delete_data("mg_subscribes", array("subs_id" => "$checking->subs_id"));
            } else {
